...
export default function AdminPage() {
    ...
    async function handleSave(e: React.FormEvent) {
        ...
        try {
            if (editingId) {
                await updateDoc(doc(db, 'skill_widgets', editingId), { ...form, updated_at: serverTimestamp() });
            } else {
                await addDoc(collection(db, 'skill_widgets'), { ...form, created_at: serverTimestamp() });
            }
            setShowForm(false);
            await loadItems();
    ...
    async function handleDelete(id: string) {
        ...
        await deleteDoc(doc(db, 'skill_widgets', id));
        await loadItems();
    }
    ...
    return (
        <div>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '24px' }}>
                <div>
                    <h1 style={{ fontSize: '28px', fontWeight: 700, margin: '0 0 4px', color: '#1a1a2e' }}>Skill Widgets</h1>
                    <p style={{ color: '#666', margin: 0 }}>Manage the 3 expandable skill cards on the homepage</p>
                </div>
