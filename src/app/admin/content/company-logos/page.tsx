...
export default function AdminPage() {
    ...
    async function handleSave(e: React.FormEvent) {
        ...
        try {
            if (editingId) {
                await updateDoc(doc(db, 'company_logos', editingId), { ...form, updated_at: serverTimestamp() });
            } else {
                await addDoc(collection(db, 'company_logos'), { ...form, created_at: serverTimestamp() });
            }
            setShowForm(false);
            await loadItems();
    ...
    async function handleDelete(id: string) {
        ...
        await deleteDoc(doc(db, 'company_logos', id));
        await loadItems();
    }
    ...
    return (
        <div>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '24px' }}>
                <div>
                    <h1 style={{ fontSize: '28px', fontWeight: 700, margin: '0 0 4px', color: '#1a1a2e' }}>Company Logos</h1>
                    <p style={{ color: '#666', margin: 0 }}>Manage supported company/partner logos</p>
                </div>
          ...
          <input style={inputStyle} type="number" value={form.order} onChange={(e) => setForm({ ...form, order: Number(e.target.value) })} />
            ...
            {loading ? (
                ...
                <div className="admin-card" style={{ textAlign: 'center', padding: '48px' }}>
                    <i className="fa-solid fa-building" style={{ fontSize: '40px', color: '#ddd', marginBottom: '12px' }}></i>
