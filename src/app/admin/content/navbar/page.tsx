...
export default function AdminPage() {
    ...
    async function loadItems() {
        setLoading(true);
        try {
            const q = query(collection(db, 'main_navbar'), orderBy('order', 'asc'));
    ...
    async function handleSave(e: React.FormEvent) {
        ...
        try {
            if (editingId) {
                await updateDoc(doc(db, 'main_navbar', editingId), { ...form, updated_at: serverTimestamp() });
            } else {
                await addDoc(collection(db, 'main_navbar'), { ...form, created_at: serverTimestamp() });
            }
            setShowForm(false);
            await loadItems();
    ...
    async function handleDelete(id: string) {
        ...
        await deleteDoc(doc(db, 'main_navbar', id));
        await loadItems();
    }
    ...
    return (
        <div>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '24px' }}>
                <div>
                    <h1 style={{ fontSize: '28px', fontWeight: 700, margin: '0 0 4px', color: '#1a1a2e' }}>Main Navbar Links</h1>
                    <p style={{ color: '#666', margin: 0 }}>Manage links that appear in the main navigation bar</p>
                </div>
          ...
          <label style={{ display: 'flex', alignItems: 'center', gap: '8px', cursor: 'pointer', marginBottom: '4px', fontSize: '13px', fontWeight: 500, color: '#555' }}>
            ...
            {loading ? (
                ...
                <div className="admin-card" style={{ textAlign: 'center', padding: '48px' }}>
                    <i className="fa-solid fa-link" style={{ fontSize: '40px', color: '#ddd', marginBottom: '12px' }}></i>
