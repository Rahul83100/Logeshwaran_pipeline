...
export default function AdminPage() {
    ...
    async function handleSave(e: React.FormEvent) {
        ...
        try {
            if (editingId) {
                await updateDoc(doc(db, 'latest_services', editingId), { ...form, updated_at: serverTimestamp() });
            } else {
                await addDoc(collection(db, 'latest_services'), { ...form, created_at: serverTimestamp() });
            }
            setShowForm(false);
            await loadItems();
        ...
        <div>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '24px' }}>
                <div>
                    ...
                    <p style={{ color: '#666', margin: 0 }}>Manage the top 3 services shown on the homepage</p>
                </div>
          ...
          <input style={inputStyle} type="number" value={form.order} onChange={(e) => setForm({ ...form, order: Number(e.target.value) })} />
            ...
            {loading ? (
                ...
                <div className="admin-card" style={{ textAlign: 'center', padding: '48px' }}>
                    <i className="fa-solid fa-briefcase" style={{ fontSize: '40px', color: '#ddd', marginBottom: '12px' }}></i>
