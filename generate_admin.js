...
const ADMIN_DIR = path.join(__dirname, 'src', 'app', 'admin', 'content');

const pages = [
  {
    folder: 'navbar',
    title: 'Main Navbar Links',
    collection: 'main_navbar',
    desc: 'Manage links that appear in the main navigation bar',
    icon: 'fa-link',
    iconBg: '#4a5568',
    interfaceSource: `
interface NavItem {
...
const emptyItem = { label: '', path: '/', showInNavbar: true, order: 0 };
`,
    formSource: `
      <div>
          <label style={labelStyle}>Label *</label>
          <input style={inputStyle} value={form.label} onChange={(e) => setForm({ ...form, label: e.target.value })} required />
      </div>
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '16px' }}>
          <div>
              <label style={labelStyle}>Path / URL *</label>
              <input style={inputStyle} value={form.path} onChange={(e) => setForm({ ...form, path: e.target.value })} required />
          </div>
          <div>
      ...
      <div>
          <label style={{ display: 'flex', alignItems: 'center', gap: '8px', cursor: 'pointer', marginBottom: '4px', fontSize: '13px', fontWeight: 500, color: '#555' }}>
...
`,
    renderSource: `
      <h4 style={{ margin: '0 0 4px', fontSize: '16px', color: '#1a1a2e' }}>{item.label}</h4>
      <p style={{ margin: '0 0 8px', fontSize: '12px', color: '#888' }}>{item.path}</p>
      <p style={{ margin: '0 0 12px', fontSize: '12px', color: item.showInNavbar ? '#48bb78' : '#a0aec0' }}>
          {item.showInNavbar ? 'Visible in Navbar' : 'Hidden from Navbar'} | Order: {item.order}
      </p>
`
  },
  {
    folder: 'latest-services',
    title: 'Latest Services',
    collection: 'latest_services',
    desc: 'Manage the top 3 services shown on the homepage',
    icon: 'fa-briefcase',
    iconBg: '#ecc94b',
    interfaceSource: `
...
const emptyItem = { title: '', description: '', order: 0 };
`,
    formSource: `
      <div>
      ...
      <div>
          ...
          <textarea style={{ ...inputStyle, height: '80px' }} value={form.description} onChange={(e) => setForm({ ...form, description: e.target.value })} required />
      </div>
      <div>
          <label style={labelStyle}>Order</label>
          <input style={inputStyle} type="number" value={form.order} onChange={(e) => setForm({ ...form, order: Number(e.target.value) })} />
...
`,
    renderSource: `
      <h4 style={{ margin: '0 0 4px', fontSize: '16px', color: '#1a1a2e' }}>{item.title}</h4>
      <p style={{ margin: '0 0 12px', fontSize: '13px', color: '#666', lineHeight: 1.4 }}>{item.description}</p>
...
`
  },
  {
    ...
    desc: 'Manage supported company/partner logos',
    icon: 'fa-building',
    iconBg: '#9f7aea',
    interfaceSource: `
interface LogoItem {
...
const emptyItem = { name: '', imageUrl: '', order: 0 };
`,
    formSource: `
      <div>
      ...
      <div>
          <ImageUploader currentUrl={form.imageUrl} onUrlChange={(url) => setForm({ ...form, imageUrl: url })} storagePath="images/company_logos" label="Company Logo *" />
      </div>
      <div>
          <label style={labelStyle}>Order</label>
          <input style={inputStyle} type="number" value={form.order} onChange={(e) => setForm({ ...form, order: Number(e.target.value) })} />
...
`,
    renderSource: `
      {item.imageUrl && (
          <div style={{ marginBottom: '12px', borderRadius: '6px', overflow: 'hidden', background: '#f5f5f5', padding: '10px' }}>
              <img src={item.imageUrl} alt={item.name} style={{ width: '100%', height: '80px', objectFit: 'contain' }} />
          </div>
      )}
      <h4 style={{ margin: '0 0 4px', fontSize: '16px', color: '#1a1a2e' }}>{item.name || 'Unnamed Company'}</h4>
      <p style={{ margin: '0 0 12px', fontSize: '12px', color: '#888' }}>Order: {item.order}</p>
`
  },
  {
    folder: 'skill-widgets',
    title: 'Skill Widgets',
    collection: 'skill_widgets',
    desc: 'Manage the 3 expandable skill cards on the homepage',
    icon: 'fa-star',
    iconBg: '#4fd1c5',
    interfaceSource: `
...
const emptyItem = { icon: 'fa-light fa-star', title: '', count: '', description: '', order: 0 };
`,
    formSource: `
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '16px' }}>
          <div>
          ...
          <div>
              <label style={labelStyle}>Count / Metric *</label>
              <input style={inputStyle} value={form.count} onChange={(e) => setForm({ ...form, count: e.target.value })} placeholder="e.g. 100+" required />
          </div>
      </div>
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '16px' }}>
          <div>
              <label style={labelStyle}>FontAwesome Icon Class</label>
              <input style={inputStyle} value={form.icon} onChange={(e) => setForm({ ...form, icon: e.target.value })} placeholder="fa-light fa-star" />
          </div>
          <div>
              <label style={labelStyle}>Order</label>
              <input style={inputStyle} type="number" value={form.order} onChange={(e) => setForm({ ...form, order: Number(e.target.value) })} />
...
`,
    renderSource: `
      <div style={{ display: 'flex', alignItems: 'center', gap: '10px', marginBottom: '8px' }}>
          <i className={item.icon} style={{ fontSize: '24px', color: '#ecc94b' }}></i>
          <h4 style={{ margin: '0', fontSize: '16px', color: '#1a1a2e' }}>{item.title}</h4>
      </div>
      <p style={{ margin: '0 0 8px', fontSize: '13px', fontWeight: 'bold' }}>{item.count}</p>
      <p style={{ margin: '0 0 12px', fontSize: '13px', color: '#666', lineHeight: 1.4 }}>{item.description}</p>
      <p style={{ margin: '0 0 12px', fontSize: '12px', color: '#888' }}>Order: {item.order}</p>
`
  },
  {
    ...
    desc: 'Manage client and partner testimonials',
    icon: 'fa-quote-left',
    iconBg: '#ed64a6',
    interfaceSource: `
...
export default function AdminPage() {
    ...
    async function loadItems() {
        setLoading(true);
        try {
            const q = query(collection(db, '${page.collection}'), orderBy('order', 'asc'));
    ...
    async function handleSave(e: React.FormEvent) {
        ...
        try {
            if (editingId) {
                await updateDoc(doc(db, '${page.collection}', editingId), { ...form, updated_at: serverTimestamp() });
            } else {
                await addDoc(collection(db, '${page.collection}'), { ...form, created_at: serverTimestamp() });
            }
            setShowForm(false);
            await loadItems();
    ...
    async function handleDelete(id: string) {
        ...
        await deleteDoc(doc(db, '${page.collection}', id));
        await loadItems();
    }
    ...
    return (
        <div>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '24px' }}>
                <div>
                    <h1 style={{ fontSize: '28px', fontWeight: 700, margin: '0 0 4px', color: '#1a1a2e' }}>${page.title}</h1>
                    <p style={{ color: '#666', margin: 0 }}>${page.desc}</p>
                </div>
            ...
            {loading ? (
                ...
                <div className="admin-card" style={{ textAlign: 'center', padding: '48px' }}>
                    <i className="fa-solid ${page.icon}" style={{ fontSize: '40px', color: '#ddd', marginBottom: '12px' }}></i>
...
pages.forEach(p => {
  const dirPath = path.join(ADMIN_DIR, p.folder);
  if (!fs.existsSync(dirPath)) {
    fs.mkdirSync(dirPath, { recursive: true });
  }
  fs.writeFileSync(path.join(dirPath, 'page.tsx'), generatePage(p));
